...
namespace RoboLib.GUI.Controls
{
    public class BindingManagerComboBox : BindingManager
    {
        ...
        protected override void OnBindToProperty()
        {
            ...
            if (BoundControl is KryptonComboBox)
            {
                var cb = BoundControl as KryptonComboBox;
        ...
        void cb_Resized(KryptonComboBox cb)
        {
            try
            {
        ...
        {
            base.OnDisposing();
            (BoundControl as KryptonComboBox).SelectionChangeCommitted -= new EventHandler(cb_SelectionChangeCommitted);
            BoundControl.KeyDown -= new KeyEventHandler(BoundControl_KeyDown);
            (BoundControl as KryptonComboBox).Resize -= (s, e) => cb_Resized(BoundControl as KryptonComboBox);
        }
    }
}
