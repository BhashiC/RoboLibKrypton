...
namespace RoboLib.GUI.Controls
{
    public partial class RTextBox : UserControl, IControlPair
    {
        ...
        {   
            // This will make sure change is only from GUI, and if Tag is null that means no bindings for the control
            if (!tbValue.Focused || this.ValueControl.Tag == null) 
            {
                return;
            }
            else
            {
                tbValue.StateCommon.Back.Color1 = Color.Yellow; ;
            }
        }
    }
}
