...
namespace RoboLib.Models
{
    public class ComponentBase : ObjBase
    {
        ...
        {
            DisplayMessage.New().Show(RUtils.MessageComp, string.Format("Initializing {0}...", this.Name));
            if (Children.Count != 0)
            {
                this.Children.ForEach(x => x.OnInitializeRecurse());
            }
            this.OnAfterInitializeRecurse();
        }
        #endregion

        #region Destroy Recurse process
        /// <summary>
        ...
        protected override void OnDestroyRecurese() 
        {
            ...
            if (Children.Count != 0)
            {
                ...
                this.Children.ForEach(x => x.OnDestroyRecurese());
            }
            DisplayMessage.New().Show(RUtils.MessageComp, string.Format("Destroying {0}...", this.Name));
        }
        #endregion
    }
    ...
    public class DisplayMessage
    {
        ...
        public static DisplayMessage New()
        {
            return new DisplayMessage();
        }

        public void Show(ComponentBase comp, string message, Severities severity = Severities.Info)
        {
            RUtils.WriteLog(severity, comp.Name, message);
        }
    }
}
