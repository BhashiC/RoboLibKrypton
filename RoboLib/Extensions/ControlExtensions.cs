...
namespace RoboLib.Extensions
{
    public static class ControlExtensions
    {
        ...
        /// <summary>
        /// Create a KryptonPage, add control T to it with full dock, then add it to the KryptonPageCollection
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="kPageCollection"></param>
        /// <param name="control"></param>
        /// <param name="pageText"></param>
        /// <param name="atFirst"></param>
        /// <returns></returns>
        public static T AddPage<T>(this KryptonPageCollection kPageCollection, T control, string pageText, bool atFirst = false) where T : Control
        {
            KryptonPage page = new KryptonPage() { Text = pageText };
            page.Controls.Add(control);
            control.Dock = DockStyle.Fill;
            if (atFirst)
            {
                kPageCollection.Insert(0, page);
            }
            else
            {
                kPageCollection.Add(page);
            }
        ...
        /// <summary>
        /// Clear and dispose pages from KryptonPageCollection (if not dispose KryptonPage will be leaked)
        /// </summary>
        /// <param name="kPageCollection"></param>
        public static void ClearNDisposePages(this KryptonPageCollection kPageCollection)
        {
            var pages = kPageCollection.ToList();
            kPageCollection.Clear();
